...
4. Codes are **generated by the app**, never taken from a segment file: random 8 digits with a per-campaign leading prefix (`5` segment, `6` calibration, `9` tests, `1`–`4` legacy demos).
5. One promocode per customer per campaign, enforced by a DB constraint.
